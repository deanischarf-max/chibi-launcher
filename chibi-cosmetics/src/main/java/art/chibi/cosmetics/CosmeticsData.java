...
public class CosmeticsData {
    ...
    public static CosmeticsData load() {
        try {
            ...
            if (dataFile == null || !Files.exists(dataFile)) {
                ChibiCosmetics.LOG.warn("[ChibiCosmetics] chibi-data.json not found");
                return null;
            }
            ...
            if (currentUser == null || !currentUser.has("name")) {
                ChibiCosmetics.LOG.warn("[ChibiCosmetics] No logged in user found");
                return null;
            }
            ...
            if (!root.has(equippedKey)) {
                ChibiCosmetics.LOG.info("[ChibiCosmetics] No equipped cosmetics for {}", name);
                return null;
            }
    ...
    private static Path findDataFile() {
        ...
        if (os.contains("win")) {
            ...
            if (appData != null) {
                path = Path.of(appData, "chibi-launcher", "chibi-data.json");
                if (Files.exists(path)) return path;
            }
        } else if (os.contains("mac")) {
            path = Path.of(home, "Library", "Application Support", "chibi-launcher", "chibi-data.json");
            if (Files.exists(path)) return path;
        } else {
            // Linux
            path = Path.of(home, ".config", "chibi-launcher", "chibi-data.json");
            if (Files.exists(path)) return path;
        }

        // Fallback: try common paths
        for (String p : new String[]{
            System.getenv("APPDATA") + "/chibi-launcher/chibi-data.json",
            home + "/.config/chibi-launcher/chibi-data.json",
            home + "/Library/Application Support/chibi-launcher/chibi-data.json",
        }) {
            try {
                path = Path.of(p);
                if (Files.exists(path)) return path;
