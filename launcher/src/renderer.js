...
// ── Cosmetics ──
function cosmIconText(id) {
  const parts = id.split('_');
  if (parts.length > 1) return parts.slice(1).join(' ').charAt(0).toUpperCase();
...
function renderCosmetics() {
  ...
  g.innerHTML=cosmetics.filter(c=>c.category===currentCat).map(c=>{
    ...
    const iconLetter=cosmIconText(c.id);
    ...
    return '<div class="'+cls+'" onclick="cosClick(\''+c.id+'\')"><div class="cosm-icon" style="background:'+grad+'"><span class="cosm-icon-letter">'+iconLetter+'</span></div><div class="cosm-name">'+c.name+'</div><div class="cosm-desc">'+c.description+'</div><div class="cosm-foot"><span class="cosm-price">&#x1FA99; '+c.price+'</span>'+badge+'</div></div>';
  }).join('');
}
